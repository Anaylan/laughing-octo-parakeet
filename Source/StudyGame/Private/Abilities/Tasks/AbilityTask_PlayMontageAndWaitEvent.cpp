...
#include "Abilities/Tasks/AbilityTask_PlayMontageAndWaitEvent.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(AbilityTask_PlayMontageAndWaitEvent)
...
void UAbilityTask_PlayMontageAndWaitEvent::OnMontageInterrupted()
{
...
void UAbilityTask_PlayMontageAndWaitEvent::EventCallback(const FGameplayEventData* Payload)
{
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
UAbilityTask_PlayMontageAndWaitEvent* UAbilityTask_PlayMontageAndWaitEvent::CreatePlayMontageAndWaitProxy(UGameplayAbility* OwningAbility,
	...
	float AnimRootMotionTranslationScale, float StartTimeSeconds, FGameplayTag EventTag)
{
	...
	if (UAbilitySystemComponent* ASC = AbilitySystemComponent.Get())
	{
		...
		if (AnimInstance != nullptr)
		{
			if (ASC->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection, StartTimeSeconds) > 0.f)
			{
				...
				InterruptedHandle = Ability->OnGameplayAbilityCancelled.AddUObject(this, &UAbilityTask_PlayMontageAndWaitEvent::OnMontageInterrupted);
				
				EventHandle =  ASC->GenericGameplayEventCallbacks.FindOrAdd(EventTag).AddUObject(this,
					&UAbilityTask_PlayMontageAndWaitEvent::EventCallback);
		...
		{
			ABILITY_LOG(Warning, TEXT("UAbilityTask_PlayMontageAndWaitEvent call to PlayMontage failed!"));
		}
	}
	else
	{
		ABILITY_LOG(Warning, TEXT("UAbilityTask_PlayMontageAndWaitEvent called on invalid AbilitySystemComponent"));
	}

	if (!bPlayedMontage)
	{
		ABILITY_LOG(Warning, TEXT("UAbilityTask_PlayMontageAndWaitEvent called in Ability %s failed to play montage %s; Task Instance Name %s."), *Ability->GetName(), *GetNameSafe(MontageToPlay),*InstanceName.ToString());
		if (ShouldBroadcastAbilityTaskDelegates())
		{
